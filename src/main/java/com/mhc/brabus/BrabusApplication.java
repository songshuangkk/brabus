...
public class BrabusApplication {

	@Bean
	CommandLineRunner demo(AccountRepository accountRepository) {
		return args -> {
			Stream.of("songshuang,spring", "dsyer,cloud", "pwebb,boot", "rwith,security")
					.map(tpl -> tpl.split(","))
	...
	public static void main(String[] args) {
		SpringApplication.run(BrabusApplication.class, args);
	}
}
...
class AccountUserDetailService implements UserDetailsService {
	...
	AccountUserDetailService(AccountRepository accountRepository) {
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		return accountRepository.findByUsername(username)
				.map(account -> new User(account.getUsername(), account.getPassword(), account.isActive(),
						account.isActive(),  account.isActive(),  account.isActive(), AuthorityUtils.createAuthorityList("ROLE_ADMIN", "ROLE_USER")))
